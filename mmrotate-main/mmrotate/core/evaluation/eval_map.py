...
def get_cls_results(det_results, annotations, class_id):
    ...
    for ann in annotations:
        ...
        import numpy as np
        if isinstance(ann['labels'], list): ann['labels'] = np.array(ann['labels'])
        if isinstance(ann['bboxes'], list): ann['bboxes'] = np.array(ann['bboxes']).reshape(-1, 5) if len(ann['bboxes']) > 0 else np.zeros((0, 5))
