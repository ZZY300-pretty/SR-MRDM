...
class DiffusionEngine(pl.LightningModule):
    ...
    def forward(self, x, batch):
        loss_out = self.loss_fn(self.model, self.denoiser, self.conditioner, x, batch)
        if isinstance(loss_out, tuple):
            loss, extra_loss_dict = loss_out
        else:
            loss, extra_loss_dict = loss_out, {}
        loss_mean = loss.mean()
        loss_dict = {"loss": loss_mean}
        for key, value in extra_loss_dict.items():
            if isinstance(value, torch.Tensor):
                loss_dict[key] = value.mean() if value.ndim > 0 else value
            else:
                loss_dict[key] = value
...
class ResidualDiffusionEngine(DiffusionEngine):
    def __init__(self, sigma_st_config, to_rgb_config, scale_01_config=None, ideal_sampler_config = None, mean_key="mu", use_flash_attn2 = False, compile_model = False, image_metrics="metrics", count_train_time=False, count_sample_time=False, image_path_key="image_path", output_path_key="output_path", save_predict_targets=True, save_predict_inputs=True, predict_output_dirname="restored",*args, **kwargs):
        if compile_model:
        ...
        self.save_predict_targets = save_predict_targets
        self.save_predict_inputs = save_predict_inputs
        self.predict_output_dirname = predict_output_dirname
    ...
    def forward(self, x, mu, batch):
        loss_out = self.loss_fn(
            self.model, self.denoiser, self.conditioner, self.sigma2st, x, mu, batch
        )
        if isinstance(loss_out, tuple):
            loss, extra_loss_dict = loss_out
        else:
        ...
        if diffusion_model is not None and hasattr(diffusion_model, "get_debug_state"):
            debug_state = diffusion_model.get_debug_state()
            for key, value in debug_state.items():
                loss_dict[f"debug/{key}"] = value
        return loss_mean, loss_dict
    ...
    def _get_sampling_model_inputs(self, batch: Dict) -> Dict:
        sampling_keys = set(getattr(self.loss_fn, "batch2model_keys", set()))
        diffusion_model = getattr(self.model, "diffusion_model", None)
        for attr_name in (
            "semantic_mask_key",
            "semantic_mask_available_key",
            "cloud_mask_key",
        ):
            key = getattr(diffusion_model, attr_name, None)
            if key is not None:
        ...
        for key in sampling_keys:
            if key in batch:
                sampling_kwargs[key] = self.get_input(batch, key)
        return sampling_kwargs

    @torch.no_grad()
    def _run_reverse_diffusion_sampling(
        self,
        batch: Dict,
        return_intermediate: bool = False,
        return_denoised: bool = False,
        ideal_sample: bool = False,
    ):
        if self.sampler is None:
            raise ValueError("Sampler must be set before running prediction.")
        ...
        sampling_kwargs = self._get_sampling_model_inputs(batch)
        ...
        if isinstance(sample_out, tuple):
            sampled_latents, others = sample_out
        else:
            sampled_latents, others = sample_out, {}

        samples = self.decode_first_stage(sampled_latents)
    ...
    def shared_test_step(self, batch):
        ...
        if self.count_sample_time:
            ...
            start_event.record()
        samples, _ = self._run_reverse_diffusion_sampling(batch)
        if self.count_sample_time:
            end_event.record()
            torch.cuda.synchronize()
        ...
        for i in range(samples.shape[0]):
            ...
            # 在 701 行上方添加这一句，从 batch 里提取条件图
            mu = batch[self.mean_key]
    ...
    def predict_step(self, batch, batch_idx):
        ...
        _, image_path_extension  = os.path.splitext(image_name)

        explicit_output_path = batch.get(self.output_path_key)
        if explicit_output_path is not None:
            output_path = explicit_output_path[0] if isinstance(explicit_output_path, (list, tuple)) else explicit_output_path
            output_path = str(output_path)
        else:
            output_path = os.path.join(self.logger.save_dir, self.predict_output_dirname, image_name)
        output_dir = os.path.dirname(output_path)
        os.makedirs(output_dir, exist_ok=True)

        label_available = batch.get("label_available", None)
        has_target = True
        if label_available is not None:
            label_available_value = label_available[0] if getattr(label_available, "ndim", 0) > 0 else label_available
            if hasattr(label_available_value, "item"):
                label_available_value = label_available_value.item()
            has_target = bool(label_available_value)

        samples = self.scale_01(samples)
        samples = torch.clamp(samples, 0.0, 1.0)

        if has_target and self.save_predict_targets:
            target = self.get_input(batch, self.input_key)
            target = self.scale_01(target)
            target = torch.clamp(target, 0.0, 1.0)
            target_rgb = np.moveaxis((self.to_rgb_func(target)[0] * 255).cpu().numpy().astype(np.uint8),0,-1)
            target_path = os.path.splitext(output_path)[0] + "_target.png"
            Image.fromarray(target_rgb).save(target_path)

        rgb = np.moveaxis((self.to_rgb_func(samples)[0] * 255).cpu().numpy().astype(np.uint8),0,-1)
        Image.fromarray(rgb).save(os.path.splitext(output_path)[0] + ".png")
        sample = samples[0].cpu().numpy()
        if image_path_extension.lower() in {".tif", ".tiff"}:
            with rasterio.open(output_path, 'w', driver='GTiff', height=sample.shape[1], width=sample.shape[2], count=sample.shape[0], dtype=sample.dtype) as dst:
                dst.write(sample)
        if self.save_predict_inputs:
            mu_path = os.path.splitext(output_path)[0] + "_mu.png"
            mu = self.scale_01(mu)
            mu = torch.clamp(mu, 0.0, 1.0)
            mu_rgb = np.moveaxis((self.to_rgb_func(mu)[0] * 255).cpu().numpy().astype(np.uint8),0,-1)
            Image.fromarray(mu_rgb).save(mu_path)
        if has_target:
            target = self.get_input(batch, self.input_key)
            target = self.scale_01(target)
            target = torch.clamp(target, 0.0, 1.0)
...
class TemporalResidualDiffusionEngine(ResidualDiffusionEngine):
    ...
    def _run_reverse_diffusion_sampling(
        ...
        with self.ema_scope("Sampling"):
            sampled_latents, others = self.sample(
                c,
                z_mu,
                batch,
                shape=z_mu.shape[1:],
    ...
    def predict_step(self, batch, batch_idx):
        ...
        c, uc = self.conditioner.get_unconditional_conditioning(
            batch,
            force_uc_zero_embeddings=conditioner_input_keys if len(self.conditioner.embedders) > 0 else []
        )
        samples, others = self._run_reverse_diffusion_sampling(
