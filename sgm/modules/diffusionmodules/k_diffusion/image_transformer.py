...
class ParallelNeighborhoodConvAttentionBlock(nn.Module):
    def __init__(
        self,
        d_model,
        d_head,
        ...
        self.n_heads = d_model // d_head
        self.kernel_size = kernel_size
        self.conv_fusion_margin = conv_fusion_margin
        self.bidirectional_attention = bidirectional_attention

        self.attn_norm = AdaRMSNorm(d_model, cond_features)
        self.qkv_proj = apply_wd(Linear(d_model, d_model * 3, bias=False))
        self.scale = nn.Parameter(torch.full([self.n_heads], 10.0))
        self.pos_emb = AxialRoPE(d_head // 2, self.n_heads)
        self.attn_dropout = nn.Dropout(dropout)
        self.attn_out_proj = apply_wd(zero_init(Linear(d_model, d_model, bias=False)))

        self.conv_norm = AdaRMSNorm(d_model, cond_features)
        self.conv_dw = apply_wd(
            nn.Conv2d(d_model, d_model, kernel_size=3, padding=1, groups=d_model, bias=False)
        )
        self.conv_pw = apply_wd(nn.Conv2d(d_model, d_model, kernel_size=1, bias=False))
        self.conv_act = nn.GELU()

        gate_hidden = max(d_model // 4, 16)
        self.channel_gate = nn.Sequential(
            apply_wd(Linear(d_model, gate_hidden, bias=False)),
            nn.SiLU(),
            apply_wd(zero_init(Linear(gate_hidden, d_model, bias=False))),
        )
        self.spatial_gate = apply_wd(zero_init(nn.Conv2d(2, 1, kernel_size=7, padding=3, bias=False)))
        self.fuse_proj = apply_wd(zero_init(Linear(d_model * 2, d_model, bias=False)))

    def extra_repr(self):
        return (
            f"d_head={self.d_head}, kernel_size={self.kernel_size}, "
            f"conv_fusion_margin={self.conv_fusion_margin}, "
            f"bidirectional_attention={self.bidirectional_attention}"
        )

    def _edge_prior(self, x):
        if self.conv_fusion_margin <= 0:
            return None
        h, w = x.shape[1], x.shape[2]
        if h <= 1 or w <= 1:
            return None
        margin = min(self.conv_fusion_margin, max(min(h, w) // 2, 1))
        if margin <= 0:
            return None
        y = torch.arange(h, dtype=x.dtype, device=x.device)
        x_coords = torch.arange(w, dtype=x.dtype, device=x.device)
        y_dist = torch.minimum(y, y.flip(0))
        x_dist = torch.minimum(x_coords, x_coords.flip(0))
        y_alpha = 1.0 - torch.clamp(y_dist / float(margin), 0.0, 1.0)
        x_alpha = 1.0 - torch.clamp(x_dist / float(margin), 0.0, 1.0)
        edge_map = torch.maximum(y_alpha[:, None], x_alpha[None, :])
        return edge_map[None, :, :, None]

    def _conv_branch(self, x, cond):
        conv_feat = self.conv_norm(x, cond).movedim(-1, -3)
        conv_feat = self.conv_dw(conv_feat)
        conv_feat = self.conv_act(conv_feat)
        conv_feat = self.conv_pw(conv_feat)
        conv_feat = conv_feat.movedim(-3, -1)
        edge_prior = self._edge_prior(conv_feat)
        if edge_prior is not None:
            conv_feat = conv_feat * (1.0 + edge_prior)
        return conv_feat

    def _attention_branch(self, x, pos, conv_feat):
        qkv = self.qkv_proj(x)
        if natten is None:
            raise ModuleNotFoundError("natten is required for neighborhood attention")
        ...
        if self.bidirectional_attention:
            conv_gate = torch.sigmoid(
                self.channel_gate(conv_feat.mean(dim=(1, 2)))
            )[:, None, None, :]
            conv_value = conv_feat * conv_gate

        if natten.has_fused_na():
            q, k, v = rearrange(qkv, "n h w (t nh e) -> t n h w nh e", t=3, e=self.d_head)
            if conv_value is not None:
                v = v + rearrange(conv_value, "n h w (nh e) -> n h w nh e", nh=self.n_heads, e=self.d_head).to(v.dtype)
            q, k = scale_for_cosine_sim(q, k, self.scale[:, None], 1e-6)
            theta = self.pos_emb(pos)
            q = apply_rotary_emb_(q, theta)
            k = apply_rotary_emb_(k, theta)
            attn_feat = natten.functional.na2d(q, k, v, self.kernel_size, scale=1.0)
            attn_feat = rearrange(attn_feat, "n h w nh e -> n h w (nh e)")
        else:
            q, k, v = rearrange(qkv, "n h w (t nh e) -> t n nh h w e", t=3, e=self.d_head)
            if conv_value is not None:
                v = v + rearrange(conv_value, "n h w (nh e) -> n nh h w e", nh=self.n_heads, e=self.d_head).to(v.dtype)
            q, k = scale_for_cosine_sim(q, k, self.scale[:, None, None, None], 1e-6)
            theta = self.pos_emb(pos).movedim(-2, -4)
            q = apply_rotary_emb_(q, theta)
            k = apply_rotary_emb_(k, theta)
            qk = natten.functional.na2d_qk(q, k, self.kernel_size)
            a = torch.softmax(qk, dim=-1).to(v.dtype)
            attn_feat = natten.functional.na2d_av(a, v, self.kernel_size)
            attn_feat = rearrange(attn_feat, "n nh h w e -> n h w (nh e)")

        attn_feat = self.attn_dropout(attn_feat)
        attn_feat = self.attn_out_proj(attn_feat)
        return attn_feat

    def forward(self, x, pos, cond):
        skip = x
        attn_input = self.attn_norm(x, cond)
        conv_feat = self._conv_branch(x, cond)
        attn_feat = self._attention_branch(attn_input, pos, conv_feat)

        if self.bidirectional_attention:
            attn_maps = attn_feat.movedim(-1, -3)
            spatial_gate = torch.sigmoid(
                self.spatial_gate(
                    torch.cat(
                        [attn_maps.mean(dim=1, keepdim=True), attn_maps.amax(dim=1, keepdim=True)],
                        dim=1,
                    )
                )
            ).movedim(-3, -1)
            conv_feat = conv_feat * (1.0 + spatial_gate)

        fused = torch.cat([attn_feat, conv_feat.to(attn_feat.dtype)], dim=-1)
...
class ParallelNeighborhoodTransformerLayer(nn.Module):
    def __init__(
        self,
        d_model,
        d_ff,
        ...
        super().__init__()
        self.self_attn = ParallelNeighborhoodConvAttentionBlock(
            d_model,
            d_head,
            cond_features,
            kernel_size,
            dropout=dropout,
            conv_fusion_margin=conv_fusion_margin,
...
class BackgroundContextAdapter(nn.Module):
    def __init__(self, d_model):
        super().__init__()
        hidden_dim = max(d_model // 2, 32)
        gate_dim = max(d_model // 4, 16)
        self.x_norm = RMSNorm(d_model)
        self.context_norm = RMSNorm(d_model)
        self.local_branch = nn.Sequential(
            apply_wd(Linear(d_model, hidden_dim, bias=False)),
            nn.SiLU(),
            apply_wd(Linear(hidden_dim, d_model, bias=False)),
        )
        ...
        self.channel_gate = nn.Sequential(
            apply_wd(Linear(d_model * 2, gate_dim, bias=False)),
            nn.SiLU(),
            apply_wd(zero_init(Linear(gate_dim, d_model, bias=False))),
        )
        self.spatial_gate = nn.Sequential(
            apply_wd(Linear(d_model * 4, hidden_dim, bias=False)),
            nn.SiLU(),
            apply_wd(zero_init(Linear(hidden_dim, d_model, bias=False))),
        )
        self.out_proj = apply_wd(zero_init(Linear(d_model, d_model, bias=False)))
        self.residual_logit = nn.Parameter(torch.tensor(-4.0))

    def forward(self, x, background_context):
        x_norm = self.x_norm(x)
        context = self.context_norm(background_context)

        local_feat = self.local_branch(x_norm)
        context_feat = self.context_branch(context)
        proposal = torch.tanh(local_feat + context_feat)

        x_pool = x_norm.mean(dim=(1, 2))
        context_pool = context.mean(dim=(1, 2))
        channel_gate = torch.sigmoid(
            self.channel_gate(torch.cat([x_pool, context_pool], dim=-1))
        )[:, None, None, :]

        spatial_features = torch.cat(
            [x_norm, context, x_norm * context, x_norm - context], dim=-1
        )
        spatial_gate = torch.sigmoid(self.spatial_gate(spatial_features))

        gated_delta = proposal * channel_gate * spatial_gate
        delta = self.out_proj(gated_delta)
        residual_scale = torch.sigmoid(self.residual_logit).to(delta.dtype)
...
class ImageTransformerDenoiserModel(nn.Module):
    def __init__(
        ...
        tanh=False,
        control_mode=None,
        use_background_context_injection=False,
        background_context_in_channels=None,
        background_context_level_indices=None,
        use_parallel_conv_fusion=False,
        parallel_conv_fusion_levels=2,
        conv_fusion_margin=16,
        bidirectional_attention=True,
    ):
        ...
        self.use_background_context_injection = use_background_context_injection
        self.use_parallel_conv_fusion = use_parallel_conv_fusion
        self.parallel_conv_fusion_levels = parallel_conv_fusion_levels
        self.conv_fusion_margin = conv_fusion_margin
        self.bidirectional_attention = bidirectional_attention
        ...
        for i, spec in enumerate(levels):
            if isinstance(spec.self_attn, GlobalAttentionSpec):
                ...
                if self.use_parallel_conv_fusion and i < self.parallel_conv_fusion_levels:
                    layer_factory = lambda _: ParallelNeighborhoodTransformerLayer(
                        spec.width,
                        spec.d_ff,
                        ...
                        conv_fusion_margin=self.conv_fusion_margin,
        ...
        if self.use_background_context_injection:
            self.background_context_in_channels = (
                background_context_in_channels if background_context_in_channels is not None else in_channels
            )
            if background_context_level_indices is None:
                background_context_level_indices = [max(len(levels) - 2, 0), len(levels) - 1]
            self.background_context_level_indices = sorted(
                {
                    int(idx)
                    for idx in background_context_level_indices
                    if 0 <= int(idx) < len(levels)
                }
            )
            self.background_patch_in = TokenMerge(
                self.background_context_in_channels, levels[0].width, patch_size
            )
            self.background_merges = nn.ModuleList(
                [TokenMerge(spec_1.width, spec_2.width) for spec_1, spec_2 in zip(levels[:-1], levels[1:])]
            )
            self.background_down_adapters = nn.ModuleDict(
                {
                    str(level_idx): BackgroundContextAdapter(levels[level_idx].width)
                    for level_idx in self.background_context_level_indices
                    if level_idx < len(levels) - 1
                }
            )
            self.background_mid_adapter = (
                BackgroundContextAdapter(levels[-1].width)
                if (len(levels) - 1) in self.background_context_level_indices
                else None
            )
        else:
            self.background_context_in_channels = None
            self.background_context_level_indices = []
            self.background_patch_in = None
            self.background_merges = None
            self.background_down_adapters = None
            self.background_mid_adapter = None
    ...
    def forward(self, x, timesteps, control=None, background_context=None):
        # Patching
        ...
        background_pyramid = None
        if self.use_background_context_injection and background_context is not None:
            background_context = background_context.movedim(-3, -1)
            background_tokens = self.background_patch_in(background_context)
            background_pyramid = [background_tokens]
            for merge in self.background_merges:
                background_tokens = merge(background_tokens)
                background_pyramid.append(background_tokens)

        # Hourglass transformer
        skips, poses = [], []
        for level_idx, (down_level, merge) in enumerate(zip(self.down_levels, self.merges)):
            x = down_level(x, pos, cond)
            if background_pyramid is not None:
                adapter_key = str(level_idx)
                if adapter_key in self.background_down_adapters:
                    x = self.background_down_adapters[adapter_key](
                        x, background_pyramid[level_idx].to(x.dtype)
                    )
            skips.append(x)
            poses.append(pos)
            x = merge(x)
        ...
        if background_pyramid is not None and self.background_mid_adapter is not None:
            x = self.background_mid_adapter(x, background_pyramid[-1].to(x.dtype))
...
class ImageTransformerDenoiserModelInterface(ImageTransformerDenoiserModel):
    def __init__(
        ...
        super().__init__(
            ...
            tanh,
            control_mode,
            use_background_context_injection=use_background_context_injection,
            background_context_in_channels=background_context_in_channels,
            background_context_level_indices=background_context_level_indices,
            use_parallel_conv_fusion=use_parallel_conv_fusion,
            parallel_conv_fusion_levels=parallel_conv_fusion_levels,
            conv_fusion_margin=conv_fusion_margin,
            bidirectional_attention=bidirectional_attention,
        )
