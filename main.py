...
class ImageLogger(Callback):
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        if self.log_first_step and pl_module.global_step == 0 and self._logged_first_step:
            return
...
if __name__ == "__main__":
    ...
    try:
        ...
        # --- 插入这两行用于 Debug ---
        print("DEBUG: config.model content is:", config.model)
        # model
        ...
        if opt.train:
            try:
                ...
                trainer.test(model, data)  # 强制进行测试/推理
            except Exception:
                if not opt.debug:
                    melk()
                raise
