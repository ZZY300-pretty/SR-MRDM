import os
import shutil
import utils.utils as utils
from utils.utils import gpu_manage
from dataload.init import get_test_data
from torch.utils.data import DataLoader
import torch
from eval import test
#from thop import profile
import time
import torch.optim as optim
import numpy as np
from utils.config import load_config


def resolve_checkpoint_path(config):
    if getattr(config, "gen_init", None) and os.path.isfile(config.gen_init):
        return config.gen_init

    out_model_dir = os.path.join(config.out_dir, "models")
    preferred_names = ["best_psnr.pth", "best_ssim.pth"]
    for name in preferred_names:
        candidate = os.path.join(out_model_dir, name)
        if os.path.isfile(candidate):
            return candidate

    epoch_candidates = []
    for candidate_dir in ["./pre_train", out_model_dir]:
        if not os.path.isdir(candidate_dir):
            continue
        for name in os.listdir(candidate_dir):
            if not name.endswith((".pth", ".pt")):
                continue
            full_path = os.path.join(candidate_dir, name)
            if name.startswith("gen_model_epoch_"):
                try:
                    epoch_num = int(os.path.splitext(name)[0].split("_")[-1])
                except ValueError:
                    epoch_num = -1
                epoch_candidates.append((epoch_num, full_path))
            else:
                epoch_candidates.append((-1, full_path))

    if epoch_candidates:
        epoch_candidates.sort(key=lambda item: item[0], reverse=True)
        return epoch_candidates[0][1]

    raise FileNotFoundError(
        "No checkpoint found. Set `gen_init` in config.yml or place a .pth file in ./pre_train."
    )

if __name__ == '__main__':
    config = load_config("config.yml")

    utils.make_manager()
    os.makedirs(config.out_dir,exist_ok=True)

    # 保存本次训练时的配置
    shutil.copyfile('config.yml', os.path.join(config.out_dir, 'config.yml'))

    gpu_manage(config)

    ### DATASET LOAD ###
    print('===> Loading datasets')
    test_dataset = get_test_data(config)
    print('test dataset:', len(test_dataset))
    validation_data_loader = DataLoader(dataset=test_dataset, num_workers=config.threads, batch_size=config.validation_batchsize, shuffle=False)
    
    ### MODELS LOAD ###
    print('===> Loading models')
    if(config.model=='spagan'):
        from model.models_spagan.gen.SPANet import Generator,SPANet
        gen = Generator(gpu_ids=config.gpu_ids,channel =config.in_ch)
    if(config.model=='amgan'):
        from model.models_amgan_cr.gen.AMGAN import Generator,AMGAN
        gen = Generator(in_ch=config.in_ch, out_ch=config.out_ch, gpu_ids=config.gpu_ids)
    if(config.model=='mdsa'):
        from model.msda.model import Generate_quarter
        gen = Generate_quarter(in_channels=config.in_ch,height=3,width=6,num_dense_layer=4,growth_rate=16)
    if(config.model=='mn'):
        from model.mn.model import MPRNet
        gen = MPRNet(in_c=config.in_ch, out_c=config.in_ch)
    if(config.model=='cvae'):
        from model.cvae.network import VAE
        gen = VAE(in_channels=config.in_ch)
    
    gen = gen.to(config.device)

    # input = torch.randn(2, 3, 256, 256).cuda()
    # all_time=[]
    # code to show the Computational complexity (speed, parameters,memory,complexity(GFLOPs))
    # for _ in range(100):
    #     time_start = time.time()
    #     predict = gen(input)
    #     time_end = time.time()
    #     all_time.append(time_end-time_start)
    # print('Speed: %.5f\n' % (1/np.mean(all_time)))
    #flops, params = profile(gen, inputs=(input, ))
    # print('Complexity: %.3fM' % (flops/1000000000), end=' GFLOPs\n')
    # optimizer = optim.SGD(gen.parameters(), lr=0.9, momentum=0.9, weight_decay=0.0005)
    # for _ in range(1000):
    #     optimizer.zero_grad()
    #     gen(input)

    checkpoint_path = resolve_checkpoint_path(config)
    param = torch.load(checkpoint_path, map_location=config.device)
    gen.load_state_dict(param)
    print('load {} as pretrained model'.format(os.path.basename(checkpoint_path)))
    criterionMSE = torch.nn.MSELoss()
    with torch.no_grad():
        log_validation = test(config, validation_data_loader, gen, criterionMSE, 200)
