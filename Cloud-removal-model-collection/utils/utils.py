...
def gpu_manage(config):
    requested_cuda = bool(getattr(config, "cuda", False))
    has_cuda = torch.cuda.is_available()
    if requested_cuda and not has_cuda:
        print("CUDA requested but not available. Falling back to CPU.")

    config.cuda = requested_cuda and has_cuda
    if config.cuda:
...
# --- 注入缺失的函数，防止报错 ---
def save_named_checkpoint(model, path, epoch, name):
    import torch
    import os
    save_path = os.path.join(path, f"{name}_epoch_{epoch}.pth")
    torch.save(model.state_dict(), save_path)
    print(f"===> Checkpoint saved at: {save_path}")

# --- 如果还有别的缺失，也可以一并在这里补充 ---
# 比如如果还报错缺少 gpu_manage，也可以在这里定义：
def gpu_manage(config):
    pass
